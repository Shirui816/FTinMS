...
for i, line in enumerate(meta_file):
    ...
    if period > 0:
        ...
        print(xi_mean_w, xi_var_w)
        #print(window_data_uwp.mean())
        #print(xi_var_w - np.cov(window_data_uwp.T, ddof=0))
    ...
    if mode == 'kde':
        kde = gaussian_kde(window_data_uwp)
...
counter = 0
while True:
    pu_xis = np.sum(pb_w_xis, axis=0) / (f_w.dot(bias_w_xis))
