...
def rdf_of_ab(a, b, box, ibox, da, db, bs, rc, cl, cc, ret, dim):
    r"""
    :param a: positions of a, (n_pa, n_d)
    :param b: positions of b, (n_pb, n_d)
    :param box: box, (n_d,)
    :param ibox: bins, (n_d,)
    :param da: diameters of a, (n_pa,)
    :param db: diameters of b, (n_pb,)
    :param bs: binsize of rdf, double
    :param rc: r_cut of rdf, double
    :param cl: cell-list of b, (n_pb,)
    :param cc: cell-count-cum, (n_cell + 1,)
    :param ret: rdfs of (n_pa, n_bin)
    :param dim: dimension of (-1, 0, 1) vector in calculating adjacent cells
    ...
    for j in range(a.shape[1] ** 3):
        ...
        for k in range(start, end):  # particle ids in cell_j
            pid_k = cl[k]
            dij = pbc_dist_cu(a[i], b[pid_k], box) - (da[i] + db[pid_k]) / 2 + 1
            if dij < rc:
                cuda.atomic.add(ret[i], int(dij / bs), 1)
...
if __name__ == "__main__":
    ...
    _bs = 0.1
    _cl, _cc = cu_cell_list(_b, _box, _ibox, gpu=0)
    _ret = np.zeros((_a.shape[0], int(_rc / _bs)), dtype=np.int64)
    _dim = np.ones(_a.shape[1], dtype=np.int64) * 3
    with cuda.gpus[0]:
